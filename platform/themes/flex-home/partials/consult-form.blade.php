<form action="{{ route('public.send.consult') }}" method="post" id="contact-form" class="generic-form">
    ...
    @if(! RealEstateHelper::isHiddenFieldAtConsultForm('phone'))
        <div class="form-group">
            <input type="text" name="phone" class="form-control" placeholder="{{ __('Phone') }} @if (RealEstateHelper::hasEnabledFieldAtConsultForm('phone')) * @endif">
        </div>
    @endif
    @if(! RealEstateHelper::isHiddenFieldAtConsultForm('email'))
        <div class="form-group">
            <input type="text" name="email" class="form-control" placeholder="{{ __('Email') }} @if (RealEstateHelper::hasEnabledFieldAtConsultForm('email')) * @endif">
        </div>
    @endif
    <div class="form-group">
